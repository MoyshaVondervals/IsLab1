...
const PersonsPage = () => {
    ...
    const filteredData = data.filter((item) => {
        ...
        const eyeColorMatch =
            !filters.eyeColor || (item.eyeColor || '').toLowerCase().includes(filters.eyeColor.toLowerCase());
        const hairColorMatch =
            !filters.hairColor || (item.hairColor || '').toLowerCase().includes(filters.hairColor.toLowerCase());
        const passportMatch =
            !filters.passportID || (item.passportID || '').toLowerCase().includes(filters.passportID.toLowerCase());
        ...
        const locationMatch =
            !filters.location || `${locationName} ${locationCoords}`.toLowerCase().includes(filters.location.toLowerCase());
    ...
    const handleDelete = async (id) => {
        try {
            await deleteApi.deletePersonById(id, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
